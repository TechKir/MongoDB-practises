...
(async function () {
  try {
    ...
    //"$all" jest tutaj nie potrzebny - ale użyłem na potrzebe zadania.
    const forumUsersToUpgrade = collection.find({
      "badges": 
        {"$elemMatch" : {
        "name" : {"$in" : ["discussion-master", "late-responder", "frequent"]},
        "createdAt":{"$gte":new Date('2019-08-01T00:00:00')}}
        },
        "badges.name":{"$ne":"premium"}
    });

    console.log(await forumUsersToUpgrade.toArray())
    // Assertions below
